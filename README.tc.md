...
- 🔌 新建接口支持 **HTTP / WebSocket / Socket.IO / GraphQL / WebDAV / MCP / TCP / UDP / MQ**，一套工作區、一棵接口樹、一份歷史記錄統一管理
...
- 📨 **MQ 消息隊列**：支持 Kafka / RabbitMQ / RocketMQ / ActiveMQ / ZeroMQ / Pulsar / NATS 以及 MQTT 系（MQTT / EMQX / HiveMQ / Mosquitto / NanoMQ / VerneMQ），維護連接配置與 Topic 後一鍵生成生產 / 消費代碼（10+ 語言），列表按消息隊列類型展示對應圖標
...
- ℹ️ **關於頁籤**：*設置 → 關於* 用 shields.io 徽章展示項目技術棧（Tauri / Rust / React / TypeScript / Vite 等），並匯總應用依賴的全部開源組件（名稱、版本、跳轉鏈接）；離線時徽章自動降級為文字標籤
...
```bash
just init        # 安裝開發環境依賴（Rust 工具鏈 + Node.js）
...
啟動應用後選擇一個空目錄作為工作目錄，會提示「生成演示案例」，可一鍵生成用戶管理、訂單管理等演示分組與示例接口（內置 Mock 已啟用）。
