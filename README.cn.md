...
- 📨 **MQ 消息队列**：支持 Kafka / RabbitMQ / RocketMQ / ActiveMQ / ZeroMQ / Pulsar / NATS 以及 MQTT 系（EMQX / HiveMQ / Mosquitto / NanoMQ / VerneMQ），维护连接配置与 Topic 后一键生成生产 / 消费代码（10+ 语言），列表按消息队列类型展示对应图标
