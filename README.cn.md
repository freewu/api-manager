...
- 🖥️ **系统托盘**：关闭窗口最小化到托盘，托盘菜单可显示/隐藏窗口、快速启停 Mock、切换语言、退出
...
- 🔌 **多协议接口**：新建接口支持 HTTP / WebSocket / Socket.IO / GraphQL / WebDAV / MCP / TCP / UDP 多种协议，一套工作区统一管理
...
- 托盘图标左键单击：显示窗口；右键：菜单
- 托盘菜单：显示窗口 / 隐藏窗口 / 启动·停止 Mock 服务 / 检查更新 / 语言（单行子菜单，勾选切换）/ 退出
...
- `just build`（可选）产物：`src-tauri/target/release/bundle/nsis/API Manager_0.1.6_x64-setup.exe` — NSIS 安装包；`bundle/msi/API Manager_0.1.6_x64_en-US.msi` — MSI
